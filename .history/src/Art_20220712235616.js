 ...
 //   let DY=((b-y)*(distortion)+y);
  ...
  const draw = p5 => {
   ...
   // danceability = 5;
    ...
    if (energy >= 0.20 && energy < 0.40) {
      en = p5.randomGaussian(0.5,0.2);
    }
    if (energy >= 0.40 && energy < 0.60) {
      en = p5.randomGaussian(0.5,0.4);
    }
    if (energy >= 0.60 && energy < .80) {
      en = p5.randomGaussian(0.5,0.6);
    }
    if (energy >= 0.80 && energy <= 1) {
      en = p5.randomGaussian(0.5,0.8);
    }
    let middle = 0.5;
    //en = middle-0.33;
    let pz = p5.randomGaussian(0.5,0.2);

 //   p5.fill(red, sat, bal);
 drawTriangle3(p5, 400,400, 400,0, 0,0, danceability, vl, pz);
  //  p5.fill(blue, sat, bal);
    drawTriangle3(p5, 0,0, 0,400, 400,400, danceability, vl, pz);
