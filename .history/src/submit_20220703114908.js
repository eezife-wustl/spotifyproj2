...
function Submit(props) {
   ...
   // const [audio, setAudio] = useState([]);
    ...
    const handleSubmit = (e) => {
        ...
        // console.log("test1")
        fetch('/playlisttracks/?playlist=37i9dQZF1DXdPec7aLTmlC')
    ...
    return (
      <div>
          ...
          {status ?  
          <>
          <a id="startButton" className="start" href="http://localhost:3001/login" onClick={hideButton}>Login</a>
          <form onSubmit={handleSubmit}>
           <input type="submit" value="Data Data" className="submitbutton"/>
          </form> 
          </>
          : 
          <a id="startButton" className="start" href="http://localhost:3001/login" onClick={hideButton}>Start Here</a> }
          ...
          {/* {items.map((track, index) => (
            ...
            {isLoaded ? <Art audiodata={sound}/> : 'not calculated'}
      </div>
    );
  }
