...
function Art({audiodata}) {
  ...
  const drawTriangle3 = (p5, x, y, X, Y, a, b, level, c, distortion) => {
    ...
    let dis = Math.sqrt(Math.pow((x-a),2)+Math.pow((y-b),2));
    distortion = 0.34;
    //calculate midpoint of x coordinate (point D)
    //D=B(t-1)+Ct

let DX = (distortion/d) * (a-x) + x;
let DY = (distortion/d) * (a-x) + x;
 ...
 //   p5.fill(red, sat, bal);
  drawTriangle3(p5, 0,0, 400,0, 400,400, danceability, vl, en);
